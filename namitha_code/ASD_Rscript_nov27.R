#Autism Spectrum Disorder Methylation analysis - Blood (Within twin pair analysis - 9 twin pairs)
...
targets_gen = read.metharray.sheet(baseDir, pattern = "ASD_blood_summarysheet.csv")
...
pdf("mean_detection_ASDblood_nov27.pdf",width=14)
par(mfrow=c(1,2))
...
pdf("mds_plots_ASDblood_nov27.pdf",width=14)
par(mfrow=c(1,2))
...
axis(1, at=0:5*3 + 1.5, labels=colnames(a), tick=TRUE) 
legend("topleft", legend=c("Low","Moderate", "High"), fill=age.pal)
wilcox.test(a[,"CD8T"],c[,"CD8T"], paired=FALSE)
...
pdf("MDS_plot_normalised_ASDblood_nov27_diag.pdf",height=14,width=14)
par(mfrow=c(2,2))
plotMDS(mds1Sw, xlab="Dimension 1", ylab="Dimension 2",col=pal[as.factor(targets_gen$Diagnosis..L.0..M.1..H.2.)],dim=c(1,2), labels=targets_gen$Sample_Name)
legend("bottomright",bg="white",col=pal,cex=.7,pch=1,legend=0:1)
plotMDS(mds2Sw, xlab="Dimension 1", ylab="Dimension 3",col=pal[as.factor(targets_gen$Diagnosis..L.0..M.1..H.2.)],dim=c(1,3), labels=targets_gen$Sample_Name)
legend("bottomright",bg="white",col=pal,cex=.7,pch=1,legend=0:1)
plotMDS(mds3Sw, xlab="Dimension 2", ylab="Dimension 3",col=pal[as.factor(targets_gen$Diagnosis..L.0..M.1..H.2.)],dim=c(2,3), labels=targets_gen$Sample_Name)
legend("bottomright",bg="white",col=pal,cex=.7,pch=1,legend=0:1)
plotMDS(mds4Sw, xlab="Dimension 3", ylab="Dimension 4",col=pal[as.factor(targets_gen$Diagnosis..L.0..M.1..H.2.)],dim=c(3,4), labels=targets_gen$Sample_Name)
legend("bottomright",bg="white",col=pal,cex=.7,pch=1,legend=0:1)
dev.off()
...
pdf("PCA_heatmap_ASDblood_nov27.pdf",height=8,width=8)
...
write.csv(output[order(output$P.Value),],file="ASD_blood_top_dmps_onADOS_withintw_Nov27_limma.csv",row.names=FALSE)

top1000 = topTable(fit2_tw,coef="ADOS",num=1000, sort.by = "P")
output_1000 = merge(ann_sub,top1000,by.x="Name",by.y="row.names")
write.csv(output_1000[order(output_1000$P.Value),],file="ASD_blood_top_dmps_onADOS_Nov27_withintw_limma_top1k.csv",row.names=FALSE)
...
write.csv(output_ruv[order(output_ruv$p.ebayes),],file="ASD_blood_topruv80%_dmps_onADOS_withintw_Nov29.csv",row.names=FALSE)
write.csv(output_ruv_1000[order(output_ruv_1000$p.ebayes),],file="ASD_blood_topruv80%_1kdmps_onADOS_withintw_Nov29.csv",row.names=FALSE)
...
##to update bioconductor - from Javana (jane's email)
install.packages("BiocManager")
BiocManager::version()
...
topKEGG(gometh_kegg)
write.csv(gometh_1k, file = "GOterms_1k.csv", row.names = TRUE)
...
plot(ilogit2(mDat[rownames(mDat) == "cg11027325",]), ADOS, main = "HES6: cg11027325", ylab = "ADOS z-score", xlab = "Beta Value")
regl5<-lm(ADOS ~ ilogit2(mDat[rownames(mDat) == "cg11027325",]))
abline(regl5)
cor.test(ADOS, ilogit2(mDat[rownames(mDat) == "cg11027325",]), method = "pearson")

plot(ilogit2(mDat[rownames(mDat) == "cg20507028",]), ADOS, main = "ZNF713: cg20507028", ylab = "ADOS z-score", xlab = "Beta Value")
regl5<-lm(ADOS ~ ilogit2(mDat[rownames(mDat) == "cg20507028",]))
abline(regl5)
cor.test(ADOS, ilogit2(mDat[rownames(mDat) == "cg20507028",]), method = "pearson")

plot(ilogit2(mDat[rownames(mDat) == "cg21244116",]), ADOS, main = "EHMT1: cg21244116", ylab = "ADOS z-score", xlab = "Beta Value")
regl5<-lm(ADOS ~ ilogit2(mDat[rownames(mDat) == "cg21244116",]))
abline(regl5)
cor.test(ADOS, ilogit2(mDat[rownames(mDat) == "cg21244116",]), method = "pearson")

plot(ilogit2(mDat[rownames(mDat) == "cg21241156",]), ADOS, main = "TLE4: cg21241156", ylab = "ADOS z-score", xlab = "Beta Value")
...
diff_effectsize_tot = c(0.02383603, 0.01913442, -0.0003221395, 0.07068035, -0.05522228, 0.05076427, 0.002702124, -0.03403429, 0.06949949)
diff_effectsize_tot

#TLE4: cg21241156
plot(diff_effectsize_tot, diff_ADOS_score, main = "TLE4: cg21241156", ylab = "ADOS difference z-score", xlab = "Delta Beta Value")
...
diff_effectsize_tot_cg21244116 = c(-0.0340176, -0.001041366, 0.02669383, -0.02422778, 0.03996136, -0.03206927, -0.004060284, 0.019002, -0.04277449)
diff_effectsize_tot_cg21244116

plot(diff_effectsize_tot_cg21244116, diff_ADOS_score, main = "EHMT1: cg21244116", ylab = "ADOS difference z-score", xlab = "Delta Beta Value")
...
diff_effectsize_tot_cg20507028 = c(-0.005014283, -0.04933726, 0.0541018, -0.06395128, 0.07563989, -0.1120907, 0.004739958, 0.01840117, -0.06681291)
diff_effectsize_tot_cg20507028

plot(diff_effectsize_tot_cg20507028, diff_ADOS_score, main = "ZNF713: cg20507028", ylab = "ADOS difference z-score", xlab = "Delta Beta Value")
...
diff_effectsize_tot_cg11027325 = c(0.1334995, 0.02969043, -0.04247427, 0.1107637, -0.03854689, 0.1779549, 0.007191577, -0.03854834, 0.1683442)
diff_effectsize_tot_cg11027325

plot(diff_effectsize_tot_cg11027325, diff_ADOS_score, main = "HES6: cg11027325", ylab = "ADOS difference z-score", xlab = "Delta Beta Value")
