...
sfarires <- mitch_calc(m2, lsfari, priority="effect")
t(sfarires$enrichment_result)

lrand <-lapply(1:1034,function(i) { set.seed(i*1000) ; sample(rownames(m2),1000) })
names(lrand) <- paste("random",1:1034)
lsets <- c(lsfari,lrand)

sfarires2 <- mitch_calc(m2, lsets, priority="effect",cores=16)

head(sfarires2$enrichment_result) %>%
  kbl(caption = "Enrichment analysis of SFARI genes") %>%
  kable_paper("hover", full_width = F)

#mitch_report(res=sfarires2,outfile="sfari.html",overwrite=TRUE)
