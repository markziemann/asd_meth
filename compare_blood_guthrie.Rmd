...
Just using the directional significance value.
...
head( mitch_bl$enrichment_result,10)
sig <- subset(mitch_bl$enrichment_result,`p.adjustANOVA`<0.05)
head(sig[order(-abs(sig$s.dist)),],10)

gu <-  res_gu_df[,"gu",drop=FALSE]
mitch_gu <- mitch_calc(gu,genesets,priority="effect")
head( mitch_gu$enrichment_result,10)
sig <- subset(mitch_gu$enrichment_result,`p.adjustANOVA`<0.05)
head(sig[order(-abs(sig$s.dist)),],10)

```

Now using the median multiplied by the directional significance.

```{r,mitch_combined, eval=FALSE}
...
head(sig[order(-abs(sig$median)),],20) %>%
  kbl(caption = "Top significant REACTOMEs with CAMERA") %>%
  kable_paper("hover", full_width = F)
