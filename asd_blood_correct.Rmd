...
## Gene methylation enrichment

Spent a lot of effort to fix up the non-unique gene names in the dataset.

```{r,gme1}

output <- read.csv("ASD_blood_top_dmps_onADOS_withintw_Nov27_limma.csv")
out <- output[,c("UCSC_RefGene_Name","t")]
...
gname_col <- unname( unlist( mclapply( 1:nrow(out) , function(i) {
  gnames <- out[i,1]
  gnames <- unique(unlist(strsplit(gnames,";")))
  sapply( gnames , function(gname) { 
    gname
  } )
} , mc.cores=CORES ) ) )

dat_col <- unname( unlist( mclapply( 1:nrow(out) , function(i) {
  gnames <- out[i,1]
  gnames <- unique(unlist(strsplit(gnames,";")))
  sapply( gnames , function(gname) {   
   out[i,2]
...
df <- data.frame(gname_col,dat_col)
gn <- unique(df$gname_col)

gme_res <- mclapply( 1:length(gn), function(i) {
  g <- gn[i]
  tstats <- df[which(df$gname_col == g),"dat_col"]
  tstats <- tstats[order(tstats)]
  myn <- length(tstats)
  mymean <- mean(tstats)
  mymedian <- median(tstats)
  #plot(tstats,main=g)
  #abline(h=mymean,col="red")
  #abline(h=mymedian,col="blue")
  wt <- wilcox.test(tstats)
  res <- c(g,myn,mymean,mymedian,wt$p.value)
} , mc.cores=CORES)
...
colnames(gme_res_df) <- c("n_probes","t_mean","t_median","p.value")
tmp <- apply(gme_res_df,2,as.numeric)
rownames(tmp) <- rownames(gme_res_df)
gme_res_df <- tmp
gme_res_df <- as.data.frame(gme_res_df)
gme_res_df <- gme_res_df[order(gme_res_df$p.value),]
gme_res_df$logp <- -log10(gme_res_df$p.value )
gme_res_df$fdr <- p.adjust(gme_res_df$p.value,method="fdr")

head(gme_res_df,20) %>% 
  kbl(caption = "Top significant genes") %>% 
  kable_paper("hover", full_width = F)

sig <- subset(gme_res_df,logp>5)
nsig <- nrow(sig)

# volcano
plot(gme_res_df$t_median , gme_res_df$logp ,
...
Mitch to identify gene set enrichments.

```{r,mitch1}

m1 <- gme_res_df[,"t_median",drop=FALSE]
...
head(mres1$enrichment_result,50) %>%
  kbl(caption = "Top significant reactome gene sets") %>% 
  kable_paper("hover", full_width = F)

sig <- subset(mres1$enrichment_result,p.adjustANOVA<0.01)

sig2a <- head(subset(sig,s.dist>0),15)
sig2b <- head(subset(sig,s.dist<0),15)
sig2 <- rbind(sig2a,sig2b)


sig2 <- sig2[order(sig2$s.dist),]
sig2_s <-  sig2$s.dist
names(sig2_s) <- sig2$set

par(mar=c(5,25,3,1))
barplot(sig2_s,horiz=TRUE,las=1,col="darkgray",xlab="Enrichment score",
 cex.names=0.8,xlim=c(-1,1))
dev.off()

```

## Wilcox gene set test

As the data are heavily skewed, mitch may be biased.
...
tstats <- lapply(1:n, function(i) {
  ...
  m1[which(rownames(m1) %in% genes),"t_median"]
} )
names(tstats) <- gs
